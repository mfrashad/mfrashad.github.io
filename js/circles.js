...
(function() {
  var Circles = window.Circles = function(options) {
    ...
    this._icon           = options.icon?options.icon:" ";
  ...
  Circles.prototype = {
    ...
    _generateText: function() {
      var html =  '<div class="' + this._textWrpClass + ' circle-flex" style="position:absolute; top:0; left:0; text-align:center; width:100%;' +
        ' font-size:' + this._radius * .7 + 'px; height:' + this._svgSize + 'px; line-height:' + this._svgSize + 'px;">'+'<div class="cicle-content">'/*+this._icon*/;
      
      html += this._calculateNumber(this._canAnimate ? 0 : this._number);
      if (this._text) {
        html += '<span class="' + this._textClass + '">' + this._text + '</span></div>';
      }
      html += "</div>" ;
